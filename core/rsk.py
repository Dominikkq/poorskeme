...
async def aio_db_transactions(conn, table, transactions):    
    if not transactions:
        logger.info("No transfers to process.")
        return
    logger.info(f"Processing - TRANSACTIONS from {transactions[0]['block']} to {transactions[-1]['block']}")
    ...
    for json_object in transactions:
        rows.append((json_object['block'],
                    ...
                    json_object['to']['hash'] if json_object.get('to') and 'hash' in json_object['to'] else '0x0000000000000000000000000000000001000006',
                    json_object['value'] if json_object['value'] is not None else '0',
                    json_object['created_contract']['hash'] if json_object.get('created_contract') and 'hash' in json_object['created_contract'] else "0x",
                    json_object['status'],
...
async def aio_db_transfers(conn, table, transfers):
    if not transfers:
        logger.info("No transfers to process.")
        return

    logger.info(f"Processing - TRANSFERS from {transfers[0]['blockNumber']} to {transfers[-1]['blockNumber']}")

    rows = []
    for json_object in transfers:
...
#!important fix Nones later
async def aio_db_internals(conn, table, internals):
    if not internals:
        logger.info("No internals to process.")
        return
    logger.info(f"Processing - INTERNALS from {internals[0].get('block', 'N/A')} to {internals[-1].get('block', 'N/A')}")

    rows = []
    for json_object in internals:
        rows.append((json_object.get('block', 0),
                     json_object.get('timestamp', ''),
                     json_object.get('transaction_hash', ''),
                     json_object['from'].get('hash', '') if json_object.get('from') else '',
                     json_object['to'].get('hash', '') if json_object.get('to') else '0x0000000000000000000000000000000000000000',
                     json_object.get('value', 0),
                     json_object.get('contractAddress', ''),
                     json_object.get('input', ''),
                     json_object.get('type', ''),
                     json_object.get('gas_limit', 0),
                     json_object.get('gasUsed', 0),
                     json_object.get('isError', 0),  # Ensure isError has a default value
                     json_object.get('errCode', '')))
...
def fetch_transactions(contract_address):
    ...
    while has_more_pages:
        try:
            ...
            batch_transactions = data.get('items', [])
            ...
            # Determine the first and last transaction blocks
            ...
            # Update params for the next page if available
...
def fetch_transfers(contract_address):
    base_url = f'https://rootstock.blockscout.com/api/v2/addresses/{contract_address}/token-transfers'
    logger.info("Starting to retrieve token transfers for the contract address")
    ...
    while has_more_pages:
        try:
            ...
            batch_transfers = data.get('items', [])
            logger.debug(f"Received {len(batch_transfers)} token transfers")

            if not batch_transfers:
                logger.info("No token transfers received, ending pagination.")
                break
...
def fetch_internal_transactions(contract_address):
    base_url = f'https://rootstock.blockscout.com/api/v2/addresses/{contract_address}/internal-transactions'
    logger.info("Starting to retrieve internal transactions for the contract address")
    ...
    while has_more_pages:
        try:
            ...
            batch_internals = data.get('items', [])
            logger.debug(f"Received {len(batch_internals)} internal transactions")

            if not batch_internals:
                logger.info("No internal transactions received, ending pagination.")
                break

            internals.extend(batch_internals)
...
async def async_fetch_and_store(contract_address, conn, table):
    ...
    if table == "t_transactions":
        transactions = fetch_transactions(contract_address)
        await aio_db_transactions(conn, table, transactions)
    elif table == "t_internals":
        internals = fetch_internal_transactions(contract_address)
        await aio_db_internals(conn, table, internals)
    elif table == "t_transfers":
        transfers = fetch_transfers(contract_address)
        logger.info(transfers)
        await aio_db_transfers(conn, table, transfers)
...
def rsk_db_collect_async(contract_address, block_from, block_to, key, filedb, chunk=30000):
    ...
    sql_create_transactions_table = """CREATE TABLE t_transactions (
    ...
    start_time = time.time()
    asyncio.run(async_fetch_and_store(contract_address, connection, "t_transfers"))
    end_time = time.time()
    elapsed_time = end_time - start_time
    ...
    logger.info(f" Elapsed time: {elapsed_time} seconds")
    logger.info(f"=========================================================")
    logger.info("Collecting Internals transfers...")
    logger.info("=====================================================")
    ...
    start_time = time.time()
    asyncio.run(async_fetch_and_store(contract_address, connection, "t_internals"))
    end_time = time.time()
    elapsed_time = end_time - start_time
    ...
    logger.info("Starting to retrieve transactions for the contract creator 2")
    ...
    start_time = time.time()
    asyncio.run(async_fetch_and_store(contract_address, "Transactions", "t_transactions_wallet"))
    end_time = time.time()
    elapsed_time = end_time - start_time
    ...
    start_time = time.time()
    asyncio.run(async_fetch_and_store(contract_address,"Transfers", "t_transfers_wallet"))
    end_time = time.time()
    elapsed_time = end_time - start_time
    ...
    start_time = time.time()
    asyncio.run(async_fetch_and_store(contract_address, "Internals", "t_internals_wallet"))
    end_time = time.time()
    elapsed_time = end_time - start_time
    # Get total transactions registered
...
def rsk_db_process(filename):
    ...
    df_trx_1 = df_trx_0[['timestamp', 'hash', 'from_hash', 'to_hash', 'value', 'method_call', 
                         'isError']]
    ...
    if (not df_i.empty):
        df_int_0 = df_i
        logger.info(df_int_0.columns)
        df_int_1 = df_int_0[['timeStamp', 'hash', 'from_address', 'to_address', 'value', 'input',
                             'is_error']]
    ...
    logger.info("HERE:")
    logger.info(df_transaction.columns)
    contract_creator = df_transaction["from_hash"]
    ...
    if (native):
        ...
        logger.info("HER2:")
        logger.info(dftemp.columns)
        unique_wallets = len(dftemp['from_hash'].unique())

        for i in dftemp.index: 
            value_raw = dftemp["value"][i]
            value = round(value_raw / 1e+18, 2)
            address_from = dftemp["from_hash"][i]
            address_to = dftemp["to_hash"][i]
    ...
    input_constructor = df_transaction["method_call"]
    input_column = df_transaction["parameters"][1:]

    decoder = InputDecoder(ABI)
    logger.info(ABI)

    # Check if the ABI contains a constructor before trying to decode it
    constructor_call = None
    try:
        constructor_call = decoder.decode_constructor(input_constructor)
    except InputDataError:
        logger.warning("Constructor is not found in ABI")
    ...
    for i in input_column:
        try:
            ...
            logger.warning(f"Function decoding failed: {e}")

    # Ensure functions and arguments have the same length as df_hash
    if len(functions) == 0:
        functions = [""] * len(df_hash)
    if len(arguments) == 0:
        arguments = [""] * len(df_hash)
    ...
    # Convert first_date and last_date to strings
    json_stats = {
        ...
        "native": int(native),
        "creator": contract_creator.iloc[0] if isinstance(contract_creator, pd.Series) else contract_creator,
        "max_liq": round(max_liq_raw / 1e18, 2),
        "max_liq_date": 0,  # !important fix max_liq_date.strftime("%Y/%m/%d - %H:%M:%S"),
    ...
    if (native):  # NOTE: For native
        trans_in = len(df_transaction)
        trans_out = len(dftemp)
    else:
